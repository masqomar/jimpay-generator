    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <select class="form-select @error('description') is-invalid @enderror" name="description" id="description" class="form-control" required>
                <option value="" selected disabled>-- {{ __('Pilih Keterangan') }} --</option>

                <option value="Tarik Simpanan Wajib">Tarik Simpanan Wajib</option>
                <option value="Tarik Simpanan Sukarela">Tarik Simpanan Sukarela</option>
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <select class="form-select @error('status') is-invalid @enderror" name="status" id="status" class="form-control" required>
                ...
                <option value="0" {{ isset($userSavingTransaction) && $userSavingTransaction->status == '0' ? 'selected' : (old('status') == '0' ? 'selected' : '') }}>{{ __('Diproses') }}</option>
                <option value="1" {{ isset($userSavingTransaction) && $userSavingTransaction->status == '1' ? 'selected' : (old('status') == '1' ? 'selected' : '') }}>{{ __('Sukses') }}</option>
            </select>
            @error('status')
            <span class="text-danger">
                {{ $message }}
